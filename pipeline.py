...
@transform_pandas(
    ...
    person_region=Input(rid="ri.foundry.main.dataset.4275c731-8425-403c-a38e-f40e0b462b65")
)
def inpatient_ml_dataset(inpatients, inpatient_charlson2, inpatient_worst_labs, inpatient_payer, person_region):
    df = inpatients
    ...
    rdf = rdf.withColumnRenamed('person_id', 'p_id')
    ...
    df = df.join(rdf, rdf.p_id == df.person_id, 'left_outer')
    df = df.drop('p_id')
...
def missing_data_info(inpatient_encoded, missing_charlson, missing_gen_eth_race):
    ...
    missing_df = missing_df[~missing_df.variable.isin(['race_white', 'race_other', 'race_native_hawaiian_or_other_pacific_islander', 'race_missing_unknown', 'race_missing_unknown', 'race_black_or_african_american', 'race_asian'])]
